...
begin
  ...
  begin
    ...
    btnStart.Visible := false; // set buttons that are enable and disabled
...
begin
  ...
  // adjust timer label to show user how time is decreasing
  if iTimerSS = 0 then
  begin
    if iTimerMM = 0 then
    begin
  ...
  if bTimerDone = true then // if the timer is done, disable the TTimer componeent and play a sound to warn user that timer is done along with a message box telling user
  begin
    timTimer.Enabled := false;
